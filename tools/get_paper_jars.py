...
def fetch_paper_versions(quiet=False):
    ...
    if success:
        ...
        if json_output:
            return {
                ...
                "message": message,
                "size": size,
                "path": str(SERVER_EXECUTABLES_DIR / f'paper-{version}-{build}.jar')
            }
        return True
    else:
        if not json_output:
...
Examples:
  ...
  python get_paper_jars.py --list --json
...
def main():
    ...
    versions = fetch_paper_versions(quiet=json_output)
    if not versions:
        if json_output:
            print(json.dumps({"success": False, "error": "Could not fetch Paper versions"}))
        else:
    ...
    if not server_info:
        if json_output:
            print(json.dumps({"success": False, "error": "No Paper server JARs found"}))
        else:
